...
async def test_visible_offcentre_emits_pan_tilt_steer_drive() -> None:
    planner = FollowPlanner(
        "d", target_distance_cm=60.0, pan_gain=0.5, tilt_gain=0.5, steer_gain=2.0, speed_kp=1.0
    )
    ...
    # Body steers toward (pan_offset 0 + frame_bearing 20): 90 + 2*20 = 130,
    # clamped to the ±30° cap → 120.
    assert _action(plan, "steer")["params"]["angle_deg"] == pytest.approx(120.0)
...
async def test_camera_pan_recentres_as_body_turns_in() -> None:
    ...
    planner = FollowPlanner("d", pan_gain=0.5)
...
async def test_centred_target_holds_camera_still() -> None:
    ...
    planner = FollowPlanner("d", pan_gain=0.5, tilt_gain=0.5, center_deadband_deg=4.0)
...
async def test_steer_capped_at_max_steer_deg() -> None:
    """Steering is clamped to ±max_steer_deg around centre."""
    planner = FollowPlanner(
        "d", pan_gain=0.5, center_deadband_deg=0.0, steer_gain=2.0, max_steer_deg=30.0
    )
